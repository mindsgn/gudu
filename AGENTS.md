...
- **Entry point**: `entrypoint.js` (not App.tsx). Imports `fast-text-encoding` then `expo-router/entry`.
- **Routing**: file-based via `expo-router`. Routes live in `app/`.
- **Database**: local SQLite via `expo-sqlite` + `drizzle-orm`. Schema: `db/schema.ts`. Migrations: `drizzle/`. Client: `db/client.ts` (db name: `gudu.db`).
- **Path alias**: `@/*` resolves to project root (e.g., `@/db/client`).
- **Shared UI**: `shared/ui/` (organisms, micro-interactions), `components/shared/`.
...
- `API_SPEC.md` — External APIs
- `DATA_MODELS.md` — SQLite schema, app types

### Quality Specs
- `SECURITY_RULES.md` — Data handling, secrets, permissions
...
- `ENVIRONMENT_CONFIG.md` — Config files

### Feature Specs
- `features/feature-template.md` — Template for new features
- `features/authentication.md` — Splash screen, auto-redirect
- `features/profile.md` — Home screen, course list
- `features/payments.md` — Learning flow (lesson, completion)
