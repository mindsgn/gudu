# 94. Hexagonal Architecture and Dependency Boundaries
...
- Difficulty: 8/10
- Prerequisites: [93. Domain Driven Design and Bounded Contexts](93-domain-driven-design-and-bounded-contexts.md), [92. Monoliths Modular Monoliths and Microservices](92-monoliths-modular-monoliths-and-microservices.md)
- File name: `94-hexagonal-architecture-and-dependency-boundaries.md`

## 1. Chapter Introduction

### What This Topic Is

Hexagonal Architecture and Dependency Boundaries is about placing domain logic at the center and pushing technology details to the edges. In a backend system, this topic matters because real services are not only business logic; they are also concrete programs and protocols moving through machines, networks, data stores, and operational constraints.

### Why Backend Engineers Need It

Expert backend work is less about a single endpoint and more about shaping systems so they remain understandable, operable, and changeable for years. If you understand hexagonal architecture and dependency boundaries only at the framework level, you can build demos. If you understand it from first principles, you can debug outages, choose better trade-offs, and explain your design to other engineers.
...
It builds on: [93. Domain Driven Design and Bounded Contexts](93-domain-driven-design-and-bounded-contexts.md), [92. Monoliths Modular Monoliths and Microservices](92-monoliths-modular-monoliths-and-microservices.md)

It is directly reinforced by: [93. Domain Driven Design and Bounded Contexts](93-domain-driven-design-and-bounded-contexts.md)

It prepares you for: [95. Event Sourcing CQRS and Auditability](95-event-sourcing-cqrs-and-auditability.md)
...
Treat hexagonal architecture and dependency boundaries as a design boundary. The main job is to decide which complexity belongs inside the boundary and which complexity must stay visible so teams can reason about it.
...
It exists because backend systems need a repeatable, understandable way to reason about placing domain logic at the center and pushing technology details to the edges without relying on folklore or fragile one-off code.

### Problems It Solves

- Turns hexagonal architecture and dependency boundaries from a vague concept into something engineers can measure, debug, and review.
...
Architecture is applied trade-off management: deciding where responsibilities live, how data flows, how failures are isolated, and how teams can change the system safely. When you learn hexagonal architecture and dependency boundaries from first principles, you are learning where the bytes go, who owns the state, and what work the computer must actually perform.
...
Think of hexagonal architecture and dependency boundaries as a flow of state through cooperating components. Each component owns part of the work, and each handoff introduces latency, failure risk, and usually some translation of data or intent.
...
In a small application, hexagonal architecture and dependency boundaries often appears in its simplest form: one process, one database, a few endpoints, and limited traffic. This is the best place to learn the shape of the concept before scale adds noise.
...
At large scale, hexagonal architecture and dependency boundaries becomes a business concern as well as a technical one. Tail latency, blast radius, recovery speed, cost, and organizational ownership all matter as much as raw correctness.
...
- `capacity models`
        ...
        - Start with a tiny working example so you can observe hexagonal architecture and dependency boundaries directly before you hide it behind frameworks or abstractions.
...
- Treating hexagonal architecture and dependency boundaries as a vocabulary item instead of a behavior that must be traced end to end.
...
- Reliability: decide what a safe failure looks like when hexagonal architecture and dependency boundaries goes wrong.
...
- Interview question: explain hexagonal architecture and dependency boundaries from first principles to a teammate who knows only high-level frameworks.
...
- Define hexagonal architecture and dependency boundaries in your own words without using jargon from the chapter.
...
- Build a small project where hexagonal architecture and dependency boundaries is central rather than incidental.
...
- You learned what hexagonal architecture and dependency boundaries is, why backend engineers care about it, and how it behaves below the abstraction layer.
...
The next useful layer is event sourcing cqrs and auditability, because it builds on the same model and extends it into a larger system concern.
